...
class Experiment:
    ...
    def __init__(self,
        ...
        # at midpoint after an emergency stop / cancel. Encoding is
        # board-specific so we compute it here (via float2bin +
        # col2buf) rather than hard-coding it server-side. Skipped if
        # init_gpa is False: in that case we trust an earlier
        # Experiment to have already registered them on this server.
        if init_gpa:
            if grad_board == 'ocra1':
                base_col = 9  # see marcompile.col2buf
            else:
                base_col = 5
            zero_words = []
            for ch in range(self.gradb.grad_channels if hasattr(self.gradb, 'grad_channels') else 4):
                dac_bin = self.gradb.float2bin(np.zeros(1), channel=ch)
                _, (msb, lsb), _ = fc.col2buf(base_col + ch, dac_bin)
